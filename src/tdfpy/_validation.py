...
def nonnegative(name: str, value: float) -> None:
    if isinstance(value, bool) or not isinstance(value, Real) or not np.isfinite(value) or value < 0:
        raise TdfpyError(f"{name} must be finite and nonnegative.")
...
def integer(name: str, value: int, minimum: int | None = 0) -> None:
    if isinstance(value, bool) or not isinstance(value, Integral) or (minimum is not None and value < minimum):
        raise TdfpyError(f"{name} must be an integer" + (f" >= {minimum}." if minimum is not None else "."))


def choice(name: str, value: str, allowed: tuple[str, ...]) -> None:
    if value not in allowed:
        raise TdfpyError(f"{name} must be one of {allowed}, got {value!r}.")


def arrays(*values: np.ndarray) -> None:
    if any(not isinstance(a, np.ndarray) or a.ndim != 1 for a in values):
        raise TdfpyError("Peak arrays must be one-dimensional NumPy arrays.")
    if len({a.size for a in values}) != 1:
        raise TdfpyError("Peak arrays must have equal lengths.")
    if any(not np.issubdtype(a.dtype, np.number) or np.iscomplexobj(a) for a in values):
        raise TdfpyError("Peak arrays must contain real numbers.")
    if any(not np.all(np.isfinite(a)) for a in values):
        raise TdfpyError("Peak arrays must contain finite values.")
...
def index_arrays(scan: np.ndarray, tof: np.ndarray, intensity: np.ndarray, num_scans: int) -> None:
    ...
    if not np.issubdtype(scan.dtype, np.integer) or not np.issubdtype(tof.dtype, np.integer):
        raise TdfpyError("Scan and TOF indices must have integer dtypes.")
    if np.any(scan < 0) or np.any(scan >= num_scans) or np.any(tof < 0) or np.any(intensity < 0):
        raise TdfpyError("Peak indices or intensities are outside their valid range.")
...
def bounds(name: str, value: tuple[float, float] | None) -> None:
    if value is None:
        return
    if len(value) != 2:
        raise TdfpyError(f"{name} must contain two bounds.")
    if not np.all(np.isfinite(value)) or value[0] < 0 or value[1] < value[0]:
        raise TdfpyError(f"{name} must have finite, nonnegative, ordered bounds.")
