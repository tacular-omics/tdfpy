...
def test_discovery_schema_metadata_and_half_open_frame_queries(service):
    ...
    with pytest.raises(ValueError, match="Unknown column"):
        service.read_table("example_dda.d", "Frames", None, [Predicate(column="bad")], 0, 1)
...
def test_entity_queries_page_and_select_actual_ids(service, mode, kind):
    ...
    selected = service.entities(path, kind, Interval(lower=rt, upper=rt), None, None, 0, 1)
...
def test_spectrum_exports_match_python_api(service, config, mode, reader_cls, raw):
    ...
    with reader_cls(DATA / path) as reader:
        ...
        expected = frame.raw_peaks() if raw else frame.centroid(centroid=MergePeaksCentroider(max_peaks=10))
...
def test_window_extraction_and_batch_export(service, config, mode, reader_cls, kind, attribute):
    path = f"example_{mode}.d"
    indices = [1, 0, 2]
    ...
    with (
        ...
        for number, index in enumerate(indices):
            expected = windows[index].centroid(centroid=MergePeaksCentroider(max_peaks=10))
    ...
    with reader_cls(DATA / path) as reader:
        np.testing.assert_array_equal(
            peaks,
            next(iter(getattr(reader, attribute))).centroid(centroid=MergePeaksCentroider(max_peaks=10)),
        )


def test_precursor_uses_existing_picker_and_rejects_ignored_options(service, config):
    row = service.entities("example_dda.d", "precursor", None, None, None, 0, 1)["items"][0]
...
def test_physical_output_selection_is_explicit_and_empty_is_valid(service):
    ...
    full, _ = service.spectrum("example_dia.d", SpectrumSelection(kind="frame", id=1), Processing(mode="raw"))
    mask = (full[:, 0] >= 500) & (full[:, 0] < 600) & (full[:, 2] >= 0.8) & (full[:, 2] < 1.2)
...
def test_calibration_conversions_match_frame_and_roundtrip_ccs(service):
    ...
    roundtrip = service.convert("example_dia.d", 1, "ccs_to_ook0", ccs["values"], 500, 2)
...
def test_paths_and_artifact_failures_do_not_modify_sources(service, tmp_path, monkeypatch):
...
def test_protocol_tool_schemas_errors_resources_and_prompts(tmp_path):
    async def run():
        ...
        async with Client(server, read_timeout_seconds=15) as client:
            ...
            bad = await client.call_tool("query_frames", {"acquisition": "example_dia.d", "limit": 10000})
            assert bad.is_error
...
def test_stdio_server_launch_and_real_extraction(tmp_path):
    async def run():
        ...
        async with Client(params, read_timeout_seconds=20) as client:
            query = await client.call_tool("query_dia_windows", {"acquisition": "example_dia.d", "limit": 1})
...
def test_filter_chain_and_watershed_match_core(service, mode):
    ...
    config = Processing(
        mode=mode,
        centroider=Operation(name="WatershedCentroider") if mode == "centroid" else None,
        noise=[
            Operation(name="DiaMs1WindowGate"),
            Operation(name="AbsoluteThreshold", parameters={"value": 100.0}),
        ],
        smoothing=Operation(name="Smooth", parameters={"scan_half_width": 0, "mz_idx_half_width": 1}),
    )
    peaks, _ = service.spectrum("example_dia.d", SpectrumSelection(kind="frame", id=1), config)
    with DIA(DATA / "example_dia.d") as reader:
        options = {
            "noise": [DiaMs1WindowGate(), AbsoluteThreshold(100)],
            "smooth": Smooth(0, 1),
        }
        expected = reader.ms1[1].raw_peaks(**options) if mode == "raw" else reader.ms1[1].centroid(centroid=WatershedCentroider(), **options)
    np.testing.assert_array_equal(peaks, expected)
