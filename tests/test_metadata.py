...
def test_metadata_mz_acq_range_is_tuple() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
...
def test_metadata_mz_acq_range_lower_less_than_upper() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
...
def test_metadata_mz_acq_range_matches_individual_properties() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        md = dda.metadata
...
def test_metadata_one_over_k0_acq_range_is_tuple() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
...
def test_metadata_one_over_k0_acq_range_lower_less_than_upper() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
...
def test_metadata_one_over_k0_acq_range_matches_individual_properties() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        md = dda.metadata
...
def test_metadata_acquisition_datetime_year() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert dda.metadata.acquisition_datetime.year == 2018


def test_metadata_acquisition_datetime_has_tzinfo() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
...
def test_metadata_unknown_key_raises_key_error() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
...
def test_calibration_date_year() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert dda.calibration.date.year == 2018


def test_calibration_date_has_tzinfo() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert dda.calibration.date.tzinfo is not None


def test_calibration_user_value() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert dda.calibration.user == "Demo User"


def test_calibration_software_value() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert dda.calibration.software == "Bruker otofControl"


def test_calibration_software_version_contains_5_1() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert "5.1" in dda.calibration.software_version


def test_calibration_mobility_calibration_date_year() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert dda.calibration.mobility_calibration_date.year == 2018


def test_calibration_mobility_calibration_user_value() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert dda.calibration.mobility_calibration_user == "Demo User"


def test_calibration_mobility_standard_deviation_percent_value() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert dda.calibration.mobility_standard_deviation_percent == pytest.approx(0.000932)


def test_calibration_reference_mobility_list_value() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        assert dda.calibration.reference_mobility_list == "Tuning Mix ES-TOF (ESI)"


def test_calibration_reference_masses_value() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
...
def test_calibration_unknown_key_raises_key_error() -> None:
    _skip_if_no_data()
    with DDA(D_PATH) as dda:
        with pytest.raises(KeyError, match="NotARealKey"):
