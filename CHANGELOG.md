...
- **`GaussianNoiseFilter`.** Structural filter that suppresses the diffuse noise halo around bright peaks via greedy non-maximum suppression in physical `(m/z, 1/K0)` space: each surviving peak drops weaker neighbours under a 2-D Gaussian envelope (defaults: peaks at 10% of the peak intensity at centre, 0.15 Da σ over a ±0.4 Da m/z half-width). Window/σ knobs follow the package's `*_half_width` naming (`mz_half_width`, `im_half_width`). Numba-JIT'd kernel with a pure-Python fallback. Promoted from the tuning dashboard.
