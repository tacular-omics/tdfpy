...
- **Noise filter subpackage (`tdfpy.noise`).** `NoiseFilter` ABC plus per-method `IntensityThreshold` subclasses (`AbsoluteThreshold`, `MadThreshold`, `PercentileThreshold`, `HistogramThreshold`, `BaselineThreshold`, `IterativeMedianThreshold`) and the structural filters `VerticalNoiseFilter` and `GaussianNoiseFilter`. Each filter exposes its tunable knobs as dataclass fields; frozen so they're hashable (Streamlit-cacheable). String/numeric shorthand (`noise="mad"`, `noise=500.0`) coerced via `coerce_filters`. Filters compose: `noise=[VerticalNoiseFilter(...), GaussianNoiseFilter(), MadThreshold(k=3)]`.
- **`GaussianNoiseFilter`.** Structural filter that clears the diffuse noise halo flanking bright peaks **along the m/z axis only** via greedy non-maximum suppression: within a `±mz_half_width` × `±im_half_width` box, each surviving peak drops weaker *m/z-offset* neighbours under a 1-D Gaussian envelope (defaults: peaks at 10% of the peak intensity, 0.15 Da σ over a ±0.4 Da m/z half-width). Same-m/z neighbours (the vertical ion-mobility streak of a real ion) are never removed; `im_half_width` only bounds the box. Numba-JIT'd kernel with a pure-Python fallback. Promoted from the tuning dashboard.
...
- **Standalone tuning dashboards** (`apps/im_feature_filter_dashboard.py`, `apps/merged_frames_dashboard.py`, `apps/raw_spectrum_dashboard.py`, `apps/raw_ms2_dashboard.py`). Their algorithms graduated into the package (`smooth` / `box_smooth`, `GaussianNoiseFilter`, `VerticalNoiseFilter`, `WatershedCentroider`); the multi-page `apps/timstof_viewer/` app is the single remaining dev tool and now calls those package APIs directly. `apps/ALGORITHM.md` is retained as the algorithm reference.
