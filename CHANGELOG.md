...
- **`GaussianNoiseFilter`.** Structural filter that suppresses the diffuse noise halo around bright peaks via greedy non-maximum suppression in physical `(m/z, 1/K0)` space: each surviving peak drops weaker neighbours under a 2-D Gaussian envelope (defaults: peaks at 30% of the peak intensity at centre, 0.15 Da σ over a 0.4 Da window). Numba-JIT'd kernel with a pure-Python fallback. Promoted from the tuning dashboard.
