"""Benchmark Rust vs Python implementation of merge_peaks."""
import time
import numpy as np
import tdfpy as tp
...
def benchmark_merge_peaks():
    ...
    print("="*80)
    print("Benchmarking Rust vs Python merge_peaks Implementation")
    print("="*80)
    ...
    with tp.timsdata_connect(TDF_PATH) as td:
        ...
        _ = _merge_peaks_rust(mz_array, intensity_array, im_array, **params)
        ...
        for i in range(n_iterations):
            start = time.perf_counter()
            rust_mz, rust_int, rust_im = _merge_peaks_rust(mz_array, intensity_array, im_array, **params)
            end = time.perf_counter()
            rust_times.append(end - start)
        
        rust_avg = np.mean(rust_times)
        rust_std = np.std(rust_times)
        rust_num_peaks = len(rust_mz)
        
        print(f"  Average time: {rust_avg*1000:.2f} ± {rust_std*1000:.2f} ms")
        print(f"  Peaks merged: {len(mz_array):,} → {rust_num_peaks:,}")
        
        # Calculate speedup
        speedup = py_avg / rust_avg
        peak_diff = abs(py_num_peaks - rust_num_peaks)
